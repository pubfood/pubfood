...
var PROVIDER_BASE_TAG_FILE = path.join(__dirname, 'provider-base-tag.js');
...
server.route({
  method: 'GET',
  path: '/simulated-provider/{provider}.js',
  handler: function routeHandler(request, reply) {
    fs.readFile(PROVIDER_BASE_TAG_FILE, function(err, file) {
      if (err) {
        console.log(err);
...
server.route({
  method: 'GET',
  path: '/simulated-provider/{provider}/refresh',
  handler: function routeHandler(request, reply) {
